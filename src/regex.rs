...
fn parse_concat(chars: &mut Peekable<Chars>) -> Result<Regex, String> {
    ...
    while let Some(&ch) = chars.peek() {
        if ch == '(' || ch.is_alphanumeric() || ch == '\\' {
            let right = parse_unary(chars)?;
            left = Regex::operation(Operation::Concat, Some(Box::new(left)), Some(Box::new(right)));
        } else if ch != ')' && ch != '|' {
            let right = parse_unary(chars)?;
            left = Regex::operation(Operation::Concat, Some(Box::new(left)), Some(Box::new(right)));
        } else {
...
pub fn regex_to_fsa(regex: &Regex) -> turing_machine::TuringMachine {
    ...
    for transition in fsa.transitions.clone().iter() {
        if transition.symbols[0] == fsa.blank_symbol {
            for symbol in fsa.input_alphabet.clone().iter() {
                fsa.add_transition(transition.state.clone(), vec![symbol.clone()], transition.new_state.clone(), vec![symbol.clone()], transition.directions.clone());
            }
        }
    }
    ...
    fsa.add_transition(begin.clone(), vec![" ".to_string()], start.clone(), vec![" ".to_string()], vec![turing_machine::Direction::Right]);
    let final_state = fsa.add_state();
    fsa.add_transition(end.clone(), vec![" ".to_string()], final_state.clone(), vec![" ".to_string()], vec![turing_machine::Direction::Stay]);
...
fn build_fsa(fsa: &mut turing_machine::TuringMachine, regex: &Regex) -> (String, String) {
    match regex.operation {
        ...
        Operation::Concat => {
            ...
            let right = regex.right.as_ref().expect("Concat must have right operand");
            
            let (start_left, end_left) = build_fsa(fsa, left);
            let (start_right, end_right) = build_fsa(fsa, right);
            
            fsa.add_transition(end_left, vec![" ".to_string()], start_right, vec![" ".to_string()], vec![turing_machine::Direction::Stay]);
        ...
        Operation::Or => {
            ...
            fsa.add_transition(start.clone(), vec![" ".to_string()], start_left.clone(), vec![" ".to_string()], vec![turing_machine::Direction::Stay]);
            fsa.add_transition(start.clone(), vec![" ".to_string()], start_right.clone(), vec![" ".to_string()], vec![turing_machine::Direction::Stay]);
            
            fsa.add_transition(end_left.clone(), vec![" ".to_string()], end.clone(), vec![" ".to_string()], vec![turing_machine::Direction::Stay]);
            fsa.add_transition(end_right.clone(), vec![" ".to_string()], end.clone(), vec![" ".to_string()], vec![turing_machine::Direction::Stay]);
            
            (start, end)
        },
        
        Operation::KleeneStar => {
            let operand = regex.left.as_ref().expect("KleeneStar must have an operand");
            ...
            fsa.add_transition(start.clone(), vec![" ".to_string()], end.clone(), vec![" ".to_string()], vec![turing_machine::Direction::Stay]);
            
            fsa.add_transition(start.clone(), vec![" ".to_string()], start_operand.clone(), vec![" ".to_string()], vec![turing_machine::Direction::Stay]);
            
            fsa.add_transition(end_operand.clone(), vec![" ".to_string()], start_operand.clone(), vec![" ".to_string()], vec![turing_machine::Direction::Stay]);
            
            fsa.add_transition(end_operand.clone(), vec![" ".to_string()], end.clone(), vec![" ".to_string()], vec![turing_machine::Direction::Stay]);
            
            (start, end)
        },
        
        Operation::KleneePlus => {
            let operand = regex.left.as_ref().expect("KleenePlus must have an operand");
